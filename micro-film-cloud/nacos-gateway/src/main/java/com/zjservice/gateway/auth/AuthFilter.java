 ...
 */
@SuppressWarnings({"AlibabaMethodTooLong", "SameParameterValue"})
@Component
@Slf4j
@Data
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    private static final String REGIST_URL ="regist";
    ...
    private String[] skipAuthRoles;
    /** 跳过不需要验证的请求方式*/
    @Value("${auth.skip.requestMethods}")
    private String[] skipAuthRequestMethods;

    /** 让redisTemplate成为该类的私有静态属性，解决注入问题*/
    private static RedisTemplate<String, JSONObject> redisTemplate;
    @Resource
    public void setRedisTemplate(RedisTemplate<String, JSONObject> redisTemplate) {
...
//        // 跳过get请求
//        for (String getMethod : skipAuthRequestMethods){
...
//            data.put("flag", false);
//            data.put("code", 401);
//            data.put("msg", "认证失败，请登录");
//            byte[] response = (data.toJSONString()).getBytes(StandardCharsets.UTF_8);
...
//        // 获取redis中对应用户的数据
//        JSONObject userMsg = redisTemplate.opsForValue().get(userName+"_info");
//        // 判断redis中的用户登录信息是否存在
//        if (userMsg == null){
//            DataBuffer buffer = getResponseData(false, 1002, "认证失败，用户未登录", originalResponse);
//            return originalResponse.writeWith(Flux.just(buffer));
//        }
//        log.info("[{}] {}",df.format(new Date()),"获取到的用户信息 "+userMsg);
//        // 比较redis中的token和当前传递的token 是否相同
//        String tokenForRedis = userMsg.getString("Token");
//        if (!token.equals(tokenForRedis)){
//            DataBuffer buffer = getResponseData(false, 1003, "认证失败，无效token", originalResponse);
//            return originalResponse.writeWith(Flux.just(buffer));
//        }
...
//        log.info("[{}] [{}] {} {} {} {}",df.format(new Date()), "用户 " + userName,
//                "访问服务 " + gatewayUrl.getId(),
//                "访问资源 " + url,
//                "请求方式 " + method,
//                "访问者远程地址 " + remoteAddress);
//        // 异步执行日志存储
...
//        return chain.filter(mutableExchange);
     ...
     * @return 返回缓冲值
     */
    private DataBuffer getResponseData(boolean flag, int code, String msg, ServerHttpResponse originalResponse) {
        JSONObject data = new JSONObject();
        data.put("flag", flag);
        data.put("code", code);
        data.put("msg", msg);
